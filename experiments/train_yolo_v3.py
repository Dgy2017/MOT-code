...
from dataloader.dataloader import VOCDetection
from configs.data_config import DataConfig as data_cfg
from utils.gutils import LRScheduler,LRSequential
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        train_dataset = VOCDetection(root=data_cfg.detection_out_path,
                                     splits=[(2007, 'train'), ])
        val_dataset = VOCDetection(root=data_cfg.detection_out_path,
                                   splits=[(2007, 'test'), ])
...
if __name__ == '__main__':
    ...
    if args.syncbn and len(ctx) > 1:
        ...
        async_net = net
    net.reset_class(data_cfg.detection_classes)
